...
//vajuras - turn this thing off in RELEASE mode
#ifdef	__MEM_PROFILER__
...
void	*m_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	try
	{
		...
		//vajuras - hard coded breakpoints to track down leaks
		if (currentAllocationCount == 387)
		{
			_asm nop;
		}
