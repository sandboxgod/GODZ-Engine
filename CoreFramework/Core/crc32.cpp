...
//---------------------------------------------------------------------------
// CRC TABLE - THIS IS A LITTLE MEMORY CONSUMING, BUT NEEDED
...
//---------------------------------------------------------------------------
// DO A CRC32 CALCULATION ON A SPECIFIED BYTE, UPDATE THE VALUE
inline void ECRC32::GetCRC32( const BYTE byte, DWORD &dwCRC32 ){
        dwCRC32 = (( dwCRC32 ) >> 8 ) ^ CRC32Table[( byte ) ^ (( dwCRC32 ) & 0x000000FF )];
}
//---------------------------------------------------------------------------
// THIS WILL BE USED INTERNALLY TO GET THE SIZE OF A GIVEN FILE
bool ECRC32::GetFileSize( const HANDLE hFile, DWORD &dwSize ){
        ...
        try {
			...
			DWORD tempDword;
			dwSize = ECRC32::GetFileSize( hFile, tempDword );
...
//---------------------------------------------------------------------------
// CALCULATE A CRC32 VALUE ON A STRING
ULONG ECRC32::Get_CRC(const char* text) 
...
DWORD ECRC32::CRC32ByFile( LPCTSTR strFileName, DWORD &dwCRC32 ){

        // MAKE SURE WE HAVE A VALID POINTER AGAIN
        if(( !strFileName ) && ( lstrlen( strFileName ))){
                return -1;
        }
        ...
        try {
                ...
                // CALCULATE THE CRC32 SIGNATURE
                        ...
                        bSuccess = ReadFile( hFile, cBuffer, sizeof( cBuffer ), &dwBytesInOut, NULL ) == TRUE;
        ...
        // AND THIS MEANS WE THREW AN EXCEPTION
        catch( ... ){
                dwErrorCode = -1;
        }
...
//---------------------------------------------------------------------------
// THIS WILL CALCULATE THE CRC32 VALUE OF A STRUCTURE
